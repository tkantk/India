...
| `npm run build:world` | `src/data/world.json` — the muted neighbouring land beyond India's own border (Pakistan, China, Nepal, Bhutan, Bangladesh, Myanmar, Sri Lanka and more), from Natural Earth, erased against India's own depicted boundary and projected to match `geo.json`. Run `build:map` first — it reads that build's cached shapefile |
...
This repository redistributes 33 files it doesn't own — one set of map
boundaries, one set of neighbouring-country coastlines, 20 photographs and 11
sounds — and 25 of them are under licences that require the author named and
the licence linked. Those licences attach that duty to *sharing* the file,
not to displaying it, so it's owed by the repository and by the deployed site
...
generated credits files verbatim, grouped into map, neighbouring land,
...
- **Neighbouring land** (`src/data/world.json`) — Pakistan, China, Nepal,
  Bhutan, Bangladesh, Myanmar, Sri Lanka and a few more within a generous box
  around India, drawn muted and flat beneath India's own map so the picture
  stops implying India is an island — is from
  [Natural Earth](https://www.naturalearthdata.com/), public domain. No
  attribution is required; it's credited on the credits page anyway, the same
  as everything else here. Generated by `npm run build:world`, which erases
  every neighbour polygon against India's own depicted boundary rather than
  merely filtering by country name.
...
[`NOTICE`](NOTICE) and in the generated credits files. In short: the map
geometry in `src/data/geo.json` derives from DataMeet's boundaries under
CC BY 4.0; the neighbouring land in `src/data/world.json` derives from
Natural Earth, public domain; the photographs in `public/photos/` remain
